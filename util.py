...
class ContinuousThread(Thread):
    ...
    def __init__(
        self, timeout=5, target=None, group=None, name=None, args=(), kwargs=None
    ):
        """
        ...
        self._kwargs = kwargs
        Thread.__init__(
            self, args=args, kwargs=kwargs, group=group, target=target, name=name
        )
    ...
    def get_most_recent_val(self):
        ...
        try:
            ...
            print(
                "Error: You ran the search function for so short a time that it couldn't even come up "
...
def run_search_function(board, search_fn, eval_fn, timeout=5):
    ...
    eval_t = ContinuousThread(
        timeout=timeout, target=search_fn, kwargs={"board": board, "eval_fn": eval_fn}
    )
