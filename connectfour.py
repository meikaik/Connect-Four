...
class ConnectFourBoard(object):
    ...
    def do_move(self, column):
        ...
        target_col = (
            [0 for _ in range(self.board_height - len(target_col) - 1)]
            + [player_id]
            + target_col
        )
    ...
    def _chain_sets_from_cell(self, row, col):
        ...
        return [
            ...
            for x in [
                ...
                + [(row, col)]
                + self._contig_vector_cells(row, col, (-1, -1)),
                list(reversed(self._contig_vector_cells(row, col, (1, 0))))
                + [(row, col)]
                + self._contig_vector_cells(row, col, (-1, 0)),
                list(reversed(self._contig_vector_cells(row, col, (0, 1))))
                + [(row, col)]
                + self._contig_vector_cells(row, col, (0, -1)),
                list(reversed(self._contig_vector_cells(row, col, (-1, 1))))
                + [(row, col)]
                + self._contig_vector_cells(row, col, (1, -1)),
            ]
    ...
    def chain_cells(self, playerid):
        ...
        for i in range(self.board_height):
            for j in range(self.board_width):
                if self.get_cell(i, j) == playerid:
                    ret_val.update(self._chain_sets_from_cell(i, j))
    ...
    def __str__(self):
        ...
        retVal += [
            str(i) + " " + " ".join([self.board_symbol_mapping[x] for x in row])
            ...
            for callback, id, symbol in (player1, player2):
                ...
                while not has_moved:
                    try:
                        ...
                        print(
                            "Player %s (%s) puts a token in column %s"
                            % (id, symbol, new_column)
                        )
